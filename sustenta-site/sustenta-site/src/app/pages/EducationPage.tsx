...
// --- NewsAPI — queries restritas a sustentabilidade --------------------------

// Palavras-chave que DEVEM aparecer para o artigo ser relevante
const PALAVRAS_SUSTENTABILIDADE = [
  ...
  'biomassa', 'biocombustivel', 'hidrogênio verde',
  ...
  'aquecimento global', 'mudancas climaticas', 'mudança climática', 'emissao de carbono',
  ...
  'carbon', 'esg', 'green', 'emisso', 'poluição', 'poluicao',
];
...
const NOTICIAS_CATEGORIAS = [
  {
    ...
    // Query bem restrita: exige termos específicos de sustentabilidade
...
async function buscarNoticias(categoria: typeof NOTICIAS_CATEGORIAS[0]): Promise<Noticia[]> {
  ...
  url.searchParams.set('pageSize', '20'); // pede mais para filtrar depois
  ...
  const noticias: Noticia[] = (data.articles ?? [])
    .filter((a: any) => {
      ...
      // Filtra apenas artigos realmente relacionados a sustentabilidade
      return isArtigoSustentavel(a.title, a.description ?? '');
    })
    .slice(0, 8) // limita a 8 após o filtro
    .map((a: any, i: number) => ({
      id: `${categoria.id}-${i}`,
      titulo: a.title,
    ...
    <div className="space-y-4">
      <div className="flex items-start gap-2 px-4 py-3 rounded-xl bg-emerald-600/10 border border-emerald-500/20">
        ...
        <p className="text-xs text-emerald-300 leading-relaxed">
          Noticias reais sobre sustentabilidade, meio ambiente e consumo consciente via NewsAPI.
          Filtradas automaticamente para exibir apenas conteudo relevante ao tema.
        </p>
      </div>
      ...
      {!loading && !erro && noticiasFiltradas.length > 0 && (
        <>
          <p className="text-xs text-slate-500">
            {noticiasFiltradas.length} noticia{noticiasFiltradas.length !== 1 ? 's' : ''} encontrada{noticiasFiltradas.length !== 1 ? 's' : ''} sobre {catAtiva.label.toLowerCase()}
          </p>
          <div className="grid sm:grid-cols-2 gap-4">
      ...
      {!loading && !erro && noticias.length === 0 && (
        <GlassCard className="p-10 text-center space-y-3">
          ...
          <p className="text-slate-300 text-sm font-semibold">Nenhuma noticia de sustentabilidade encontrada</p>
          <p className="text-slate-500 text-xs leading-relaxed max-w-xs mx-auto">
            A NewsAPI nao retornou artigos relevantes para esta categoria no momento. Tente atualizar ou escolha outra categoria.
          </p>
          <button onClick={() => carregar(catAtiva, true)}
