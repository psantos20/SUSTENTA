...
export const MapPage: React.FC = () => {
  ...
  // Variavel dark usada para reestilizar mapa quando tema mudar
  React.useEffect(() => {
  ...
  // ─── buscarEcopontos — corrigido DistanceMatrix ───────────────────────────
  const buscarEcopontos = async (lat: number, lng: number) => {
    ...
    // 1. Geocode reverso para detectar cidade
    ...
    // 2. textSearch — queries por cidade
    ...
    // 3. nearbySearch fallback — parques
    ...
    // 4. Ordena por haversine e limita a 20
    resultados.sort((a, b) => (a.distancia || 0) - (b.distancia || 0));
    const lista = resultados.slice(0, 20);

    // 5. DistanceMatrix — CORRIGIDO: passa LatLng objects validos
    try {
      if (lista.length > 0) {
        // Cria array de LatLng objects validos (nao indices!)
        const destinos = lista.map(e =>
          new window.google.maps.LatLng(e.lat, e.lng)
        );

        const distMatrix = new window.google.maps.DistanceMatrixService();

        // Divide em chunks de 25 (limite da API)
        const CHUNK = 25;
        for (let ci = 0; ci < destinos.length; ci += CHUNK) {
          const chunk = destinos.slice(ci, ci + CHUNK); // <-- CORRIGIDO: chunk real, nao [ci]
          await new Promise<void>(resolve => {
            distMatrix.getDistanceMatrix(
              {
                origins: [center],          // LatLng object valido
                destinations: chunk,        // array de LatLng objects validos
        ...
        // Re-ordena com distancia real da API
