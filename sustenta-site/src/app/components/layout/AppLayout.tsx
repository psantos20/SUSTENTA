...
export const AppLayout: React.FC<AppLayoutProps> = ({
  ...
  const ultimasNotifs = notificacoes.slice(0, 5);
        ...
        <header className="sticky top-0 z-40 h-16 bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 px-4 md:px-8 flex items-center justify-between transition-colors duration-300">
          <div className="flex items-center gap-4">
            ...
            <div className="hidden md:flex items-center gap-3 bg-slate-100 dark:bg-slate-700 px-4 py-2 rounded-xl transition-colors">
              ...
              <input type="text" placeholder="Buscar por análises, registros..."
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100 dark:border-slate-700">
                      <div className="flex items-center gap-2">
                        <span className="font-bold text-sm text-slate-900 dark:text-slate-100">Notificações</span>
                        {naoLidas > 0 && (
                    ...
                    <div className="max-h-80 overflow-y-auto">
                      {ultimasNotifs.length === 0 ? (
                        <div className="py-8 text-center">
                          ...
                          <p className="text-xs text-slate-400 dark:text-slate-500">Nenhuma notificação</p>
                        </div>
                      ) : (
                        ultimasNotifs.map(notif => {
                          const Icon = ICONE_TIPO[notif.tipo];
                          const cor  = COR_TIPO[notif.tipo];
                          return (
                            <button key={notif.id}
                              onClick={() => { marcarLida(notif.id); setDropdownAberto(false); onNavigate('notifications'); }}
                    ...
                    {/* Footer dropdown */}
                    <button
                      onClick={() => { setDropdownAberto(false); onNavigate('notifications'); }}
                      className="w-full py-3 text-xs font-bold text-emerald-600 dark:text-emerald-400 hover:bg-emerald-50 dark:hover:bg-emerald-900/20 transition-colors border-t border-slate-100 dark:border-slate-700"
                    >
