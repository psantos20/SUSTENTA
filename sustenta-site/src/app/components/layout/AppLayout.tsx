...
export const AppLayout: React.FC<AppLayoutProps> = ({
  ...
  const paginasBusca = [
    { nome: 'Dashboard', page: 'dashboard' },
    { nome: 'Consumo', page: 'consumption' },
    { nome: 'Análises', page: 'analysis' },
    { nome: 'Hábitos', page: 'habits' },
    { nome: 'Histórico', page: 'history' },
    { nome: 'Mapa', page: 'map' },
    { nome: 'Relatórios', page: 'reports' },
    { nome: 'Educação', page: 'education' },
    { nome: 'Perfil', page: 'profile' },
  ];

  const resultadosBusca = paginasBusca.filter((item) =>
        ...
        <header className="sticky top-0 z-40 h-16 bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 px-4 md:px-8 flex items-center justify-between transition-colors duration-300">
          <div className="flex items-center gap-4">
            ...
            <div className="hidden md:block relative">
              <div className="flex items-center gap-3 bg-slate-100 dark:bg-slate-700 px-4 py-2 rounded-xl transition-colors">
              ...
              {busca && (
                <div className="absolute top-12 left-0 w-full bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-lg overflow-hidden z-50">
                  {resultadosBusca.length > 0 ? (
                    resultadosBusca.map((item) => (
                      <button
                        ...
                        className="w-full text-left px-4 py-3 text-sm text-slate-700 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors"
                      >
                        {item.nome}
                      </button>
                    ))
          ...
          <>
            ...
            >
              <div className="flex items-center justify-between p-6 border-b border-slate-100 dark:border-slate-700">
                ...
                <button
                  className="p-2 text-slate-400 dark:hover:text-slate-200"
                  onClick={() => setIsMobileMenuOpen(false)}
                >
                  <X className="w-6 h-6" />
                </button>
