...
function calcularScore(energia: number, agua: number, outro: number): number {
  ...
  if (energia > 400) score -= 30;
  else if (energia > 200) score -= 15;

  if (agua > 200) score -= 20;
  else if (agua > 100) score -= 10;
  ...
  return (
    <div className="flex items-center gap-6">
      <svg width="128" height="128" viewBox="0 0 128 128">
        ...
        <text x={cx} y={cy - 4} textAnchor="middle" fill="white" fontSize="10" opacity="0.6">
          Total
        </text>
    ...
    >
      <div className="flex items-center justify-between border-b border-slate-700 pb-6">
        ...
        <div className="text-right">
          ...
          <p className="text-xs text-slate-500">
            {data.usuario?.cidade || 'Cidade'}, {data.usuario?.estado || 'UF'}
          </p>
        </div>
      </div>
          ...
          <table className="w-full text-xs">
            ...
            <tbody>
              ...
              <tr className="border-t border-slate-600">
                <td colSpan={2} className="pt-2 text-slate-400 font-medium">
                  Total
                </td>
                <td className="pt-2 text-right text-white font-bold">
                  R$ {data.totalGeral.toFixed(2)}
                </td>
              </tr>
            </tbody>
          </table>
      ...
      <div className="border-t border-slate-700 pt-4 flex justify-between items-center">
        <p className="text-xs text-slate-600">
          Gerado pelo Sustenta em {new Date().toLocaleDateString('pt-BR')}
        </p>
...
export default function ReportsPage() {
  ...
  async function carregarDados() {
    ...
    try {
      ...
      if (totalEnergia > 200) {
        alertas.push(`Gasto com energia (R$ ${totalEnergia.toFixed(2)}) acima de R$ 200,00.`);
      }

      if (totalAgua > 100) {
        alertas.push(`Gasto com água (R$ ${totalAgua.toFixed(2)}) acima de R$ 100,00.`);
      }
      ...
      if (totalEnergia > 0) {
        dicas.push('Substitua lâmpadas comuns por LED — reduz até 80% no consumo.');
      }

      if (totalAgua > 0) {
        dicas.push('Redutores de vazão nas torneiras economizam até 50% na água.');
      }

      if (totalOutro > 200) {
        dicas.push('Revise seus gastos avulsos — pequenos cortes geram grande impacto.');
      }

      if (scoreMes < 60) {
        dicas.push('Score baixo: foque em reduzir o maior gasto deste mês.');
      }

      if (scoreMes >= 80) {
        dicas.push('Score excelente! Continue mantendo seus hábitos sustentáveis.');
      }

      setReportData({
        mes: mesSelecionado,
        mesLabel: getMesLabel(mesSelecionado),
        usuario,
        registros,
        totalGeral,
        totalEnergia,
  ...
  function handleAbrirPreviewPDF() {
  ...
  const cardBase = clsx(
    ...
    darkMode ? 'bg-slate-800/60 border-slate-700/50' : 'bg-white border-slate-200 shadow-sm'
  );
          ...
          >
            ...
            >
              {[
                ...
                <button
                  key={tab.id}
                  onClick={() => setActiveTab(tab.id as 'resumo' | 'evolucao' | 'registros')}
                  className={clsx(
      ...
      <AnimatePresence>
        {previewOpen && reportData && (
          <div
            ...
            onClick={(e) => {
              if (e.target === e.currentTarget && !generating) setPreviewOpen(false);
            }}
          >
            <motion.div
            ...
            >
              <div className="sticky top-0 z-10 flex justify-between items-center px-4 py-3 bg-slate-950/80 backdrop-blur-sm rounded-t-2xl border-b border-slate-700">
                <span className="text-sm font-semibold text-white">Preview PDF — {reportData.mesLabel}</span>

                <div className="flex gap-2">
                  <button
                    onClick={handleBaixarPDF}
                    disabled={generating}
                    className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-semibold transition-colors disabled:opacity-70 disabled:cursor-not-allowed"
                  >
                  ...
                  <button
                    ...
                    disabled={generating}
                    className="px-3 py-1.5 rounded-lg bg-slate-700 hover:bg-slate-600 text-slate-300 text-xs font-semibold transition-colors disabled:opacity-60 disabled:cursor-not-allowed"
                  >
                    Fechar
                  </button>
                </div>
