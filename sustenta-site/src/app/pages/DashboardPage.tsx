      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold text-slate-900 dark:text-slate-100">
            Dashboard Financeiro & Sustentável
          </h1>
      ...
      {carregando ? (
        ...
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2 bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm">
              <h3 className="text-lg font-bold text-slate-900 dark:text-slate-100 mb-6">
                Distribuição de Gastos em {mesLabel}
              </h3>
          ...
          <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm">
            <h3 className="text-lg font-bold text-slate-900 dark:text-slate-100 mb-6">Alertas & Insights</h3>
            <div className="space-y-3">
              {energia > 200 && (
                <div className="flex items-center gap-4 p-4 rounded-xl border border-amber-100 dark:border-amber-900/40 bg-amber-50 dark:bg-amber-900/20">
                  <AlertCircle className="w-5 h-5 text-amber-500 shrink-0" />
                  <div>
                    <p className="text-sm font-bold text-slate-800 dark:text-slate-200">Energia acima de R$ 200</p>
                    <p className="text-xs text-slate-500 dark:text-slate-400">Considere reduzir o uso de aparelhos de alto consumo.</p>
                  </div>
                </div>
              )}
              {agua > 100 && (
                <div className="flex items-center gap-4 p-4 rounded-xl border border-blue-100 dark:border-blue-900/40 bg-blue-50 dark:bg-blue-900/20">
                  <AlertCircle className="w-5 h-5 text-blue-500 shrink-0" />
                  <div>
                    <p className="text-sm font-bold text-slate-800 dark:text-slate-200">Água acima de R$ 100</p>
                    <p className="text-xs text-slate-500 dark:text-slate-400">Verifique possíveis vazamentos e reduza o tempo no banho.</p>
                  </div>
                </div>
              )}
              {Object.entries(subcategorias).map(([nome, val]) => val > 300 && (
                <div key={nome} className="flex items-center gap-4 p-4 rounded-xl border border-red-100 dark:border-red-900/40 bg-red-50 dark:bg-red-900/20">
                  <ArrowUpRight className="w-5 h-5 text-red-500 shrink-0" />
                  <div>
                    <p className="text-sm font-bold text-slate-800 dark:text-slate-200">{nome} acima de R$ 300</p>
                    <p className="text-xs text-slate-500 dark:text-slate-400">Este gasto está elevado. Considere revisar.</p>
                  </div>
                </div>
              ))}
              {totalRegistros > 0 && energia <= 200 && agua <= 100 && Object.values(subcategorias).every(v => v <= 300) && (
                <div className="flex items-center gap-4 p-4 rounded-xl border border-emerald-100 dark:border-emerald-900/40 bg-emerald-50 dark:bg-emerald-900/20">
                  <CheckCircle2 className="w-5 h-5 text-emerald-500 shrink-0" />
                  <div>
                    <p className="text-sm font-bold text-slate-800 dark:text-slate-200">Tudo dentro da meta! 🎉</p>
                    <p className="text-xs text-slate-500 dark:text-slate-400">Seus gastos estão controlados este mês. Continue assim!</p>
                  </div>
                </div>
              )}
            </div>
