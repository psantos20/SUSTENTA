...
const DESAFIOS_PREDEFINIDOS: Desafio[] = [
  ...
  },
  {
    ...
    duracaoDias: 5,
    categoria: 'Energia',
    meta: 'Complete 5 dias sem eletrônicos à noite',
  },
  {
    id: 'd4',
    titulo: 'Semana Verde',
    ...
    duracaoDias: 7,
    categoria: 'Geral',
    meta: 'Streak de 7 dias completos',
  },
  {
    id: 'd5',
    ...
    duracaoDias: 14,
    categoria: 'Resíduos',
    meta: 'Registre compostagem por 14 dias',
  },
];
...
export const HabitsPage: React.FC = () => {
  ...
  const [habitoDetalhe, setHabitoDetalhe] = React.useState<Habito | null>(null);
  ...
  const alternarHabito = (habito: Habito, feito: boolean) => {
    if (salvando === habito.id) return;
    feito ? desmarcarHabito(habito) : marcarHabito(habito);
  };
  ...
  const avancarDesafio = async (desafio: Desafio) => {
    ...
    try {
      ...
      if (!progAtual || progAtual.concluido) return;

      const novoProgresso = Math.min(
        100,
        progAtual.progresso + Math.round(100 / desafio.duracaoDias)
      );
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        ...
        <div className="flex p-1 bg-slate-100 dark:bg-slate-700 rounded-xl w-fit">
          {[
            ...
            <button
              key={tab.id}
              onClick={() => setActiveTab(tab.id as 'habitos' | 'desafios' | 'ranking')}
                ...
                {todosHabitos.map((habito) => {
                  ...
                  return (
                    <motion.div
                      ...
                      role="button"
                      tabIndex={0}
                      onClick={() => alternarHabito(habito, feito)}
                      onKeyDown={(e) => {
                        if (e.key === 'Enter' || e.key === ' ') {
                          e.preventDefault();
                          alternarHabito(habito, feito);
                        }
                      }}
                      className={`cursor-pointer flex items-center justify-between p-5 rounded-2xl border transition-all ${
                        feito
                    ...
                    >
                      <div className="flex items-center gap-4">
                        <button
                          ...
                          className="transition-transform active:scale-90 shrink-0 cursor-pointer disabled:cursor-not-allowed"
                        >
                          {carregandoEste ? (
                            ...
                            <Circle className="w-7 h-7 text-slate-300 dark:text-slate-600 hover:text-emerald-400 transition-colors" />
                          )}
                        </button>
                        ...
                        <div>
                          <div className="flex items-center gap-2 mb-1">
                            <span
                              className={`text-[9px] font-bold uppercase tracking-widest px-1.5 py-0.5 rounded flex items-center gap-1 ${cor.bg} ${cor.text}`}
                            >
                      ...
                      <div className="flex items-center gap-2">
                        {habito.personalizado && (
                          <button
                            ...
                            className="p-2 text-slate-300 dark:text-slate-600 hover:text-red-500 transition-colors rounded-xl hover:bg-red-50 dark:hover:bg-red-900/20"
                          >
                ...
                <div className="relative z-10">
                  ...
                  <p className="text-[10px] text-emerald-100 font-medium">
                    {concluidos === total && total > 0
                      ...
                      : `Complete mais ${total - concluidos} hábito${
                          total - concluidos !== 1 ? 's' : ''
                        } para ganhar bônus!`}
                  </p>
        ...
        {activeTab === 'desafios' && (
          ...
          >
                ...
                >
                  <div>
                    <div className="flex items-center justify-between mb-6">
                      <span
                        className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest border ${COR_DIFICULDADE[desafio.dificuldade]}`}
                      >
                        {desafio.dificuldade}
                      </span>

                      <span
                        className={`flex items-center gap-1 text-[10px] font-bold px-2 py-1 rounded-full ${cor.bg} ${cor.text}`}
                      >
                        ...
                        <div className="w-full h-2 bg-slate-100 dark:bg-slate-700 rounded-full overflow-hidden">
                          <div
                            className={`h-full transition-all duration-700 rounded-full ${
                              concluido ? 'bg-emerald-500' : 'bg-emerald-600'
                            }`}
                            style={{ width: `${pct}%` }}
                          />
                        </div>
                  ...
                  <div className="flex items-center justify-between pt-6 border-t border-slate-50 dark:border-slate-700">
                    ...
                    {concluido ? (
                      ...
                      <button
                        onClick={() => avancarDesafio(desafio)}
                        disabled={!!carregandoEste}
            ...
            {[1, 2].map((_, i) => (
              <div
                key={i}
                className="bg-slate-50 dark:bg-slate-800/50 p-8 rounded-3xl border border-dashed border-slate-200 dark:border-slate-700 flex flex-col items-center justify-center text-center gap-4 opacity-60"
              >
                        ...
                        <div className="flex-1">
                          ...
                          <div className="w-full h-1.5 bg-slate-100 dark:bg-slate-700 rounded-full mt-1.5 overflow-hidden">
                            <div
                              ...
                              style={{
                                width:
                                  rankingFamilia[0].xp > 0
                                    ? `${(membro.xp / rankingFamilia[0].xp) * 100}%`
                                    : '0%',
                              }}
                            />
                          </div>
            ...
            >
              <div className="bg-white dark:bg-slate-800 rounded-3xl shadow-2xl overflow-hidden border border-slate-100 dark:border-slate-700">
                ...
                <div className="p-6 space-y-4">
                  ...
                  <div className="p-4 bg-slate-50 dark:bg-slate-700 rounded-2xl">
                    ...
                    <p className="text-sm text-slate-600 dark:text-slate-300 leading-relaxed">
                      Ao concluir essa tarefa no dia de hoje, você ganha XP e contribui para sua
                      sequência diária de hábitos sustentáveis.
                    </p>
                  </div>
                </div>
                ...
                <div className="flex gap-3 px-6 pb-6">
                  ...
                  <button
                    onClick={() => {
                      const feito = feitos.some((h) => h.id === habitoDetalhe.id);
                      alternarHabito(habitoDetalhe, feito);
                      setHabitoDetalhe(null);
                    }}
                    className="flex-1 py-3 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-2xl transition-all text-sm shadow-sm shadow-emerald-200"
                  >
                    {feitos.some((h) => h.id === habitoDetalhe.id)
                      ? 'Desmarcar'
                      : 'Concluir'}
                  </button>
                </div>
              ...
              <div className="bg-white dark:bg-slate-800 rounded-3xl shadow-2xl overflow-hidden">
                ...
                <div className="p-6 space-y-4">
                  ...
                  <div className="space-y-1.5">
                    ...
                    >
                      {(['Água', 'Energia', 'Resíduos', 'Alimentação', 'Transporte', 'Geral'] as Categoria[]).map(
                        (c) => (
                ...
                <div className="flex gap-3 px-6 pb-6">
                  ...
                  <button
                    onClick={criarHabito}
                    disabled={!novoTitulo.trim() || salvando === 'novo'}
