...
function scoreLabel(s: number) {
  ...
  if (s >= 65) return 'Regular';   if (s >= 55) return 'Atenção'; return 'Crítico';
}
...
function tipoPonto(tipos: string[]): { label: string; icon: React.ElementType } {
  ...
  if (tipos.some(t => t.includes('park') || t.includes('natural')))   return { label: 'Área Verde', icon: TreePine };
  if (tipos.some(t => t.includes('electric') || t.includes('charging'))) return { label: 'Recarga EV', icon: Zap };
  if (tipos.some(t => t.includes('water') || t.includes('spring')))   return { label: "Ponto d'água", icon: Droplets };
...
export const MapPage: React.FC = () => {
  ...
  // Google Maps pronto
  React.useEffect(() => {
    if (window._googleMapsReady || window.google?.maps) { setGooglePronto(true); return; }
    const h = () => setGooglePronto(true);
    window.addEventListener('google-maps-ready', h);
    return () => window.removeEventListener('google-maps-ready', h);
  }, []);
    ...
    navigator.geolocation.getCurrentPosition(
      pos => {
        ...
        if (mapInstanceRef.current) {
          ...
          marcadorUserRef.current = new window.google.maps.Marker({ position: { lat, lng }, map: mapInstanceRef.current, icon: criarIconeUsuario(), title: 'Você está aqui', zIndex: 999 });
        }
      },
      () => { setLoadingPos(false); alert('Não foi possível obter sua localização.'); },
      { timeout: 10000 }
    );
  };

  // ─── Busca ecopontos — robusto com fallback ───────────────────────────────
  const buscarEcopontos = async (lat: number, lng: number) => {
    ...
    // 1. Detecta cidade via geocode reverso
    ...
    console.log('[Sustenta] Cidade detectada:', cidade);

    const service = new window.google.maps.places.PlacesService(mapInstanceRef.current);
    const center  = new window.google.maps.LatLng(lat, lng);
    const resultados: Ecoponto[] = [];

    // 2. textSearch — queries amplas por cidade
    ...
    await Promise.all(queries.map(q =>
      new Promise<void>(resolve => {
        service.textSearch({ query: q, location: center, radius: 30000 }, (results: any[], status: string) => {
          console.log(`[Sustenta] textSearch "${q}" → status=${status} resultados=${results?.length || 0}`);
    ...
    // 3. nearbySearch de fallback — busca parques e áreas verdes perto
    if (resultados.length < 5) {
      await new Promise<void>(resolve => {
        service.nearbySearch({ location: center, radius: 10000, type: 'park' }, (results: any[], status: string) => {
          console.log(`[Sustenta] nearbySearch park → status=${status} resultados=${results?.length || 0}`);
          if (status === 'OK' && results) {
    ...
    console.log('[Sustenta] Total antes de ordenar:', resultados.length);

    // 4. Ordena por distância haversine (fallback confiável)
    resultados.sort((a, b) => (a.distancia || 0) - (b.distancia || 0));
    const lista = resultados.slice(0, 20);

    // 5. Tenta enriquecer com DistanceMatrix (não bloqueia se falhar)
    try {
      ...
      if (destinos.length > 0) {
        ...
        // DistanceMatrix suporta max 25 destinos por chamada
        const chunks = [];
        for (let i = 0; i < destinos.length; i += 25) chunks.push(destinos.slice(i, i + 25));

        for (let ci = 0; ci < chunks.length; ci++) {
          await new Promise<void>(resolve => {
            distMatrix.getDistanceMatrix(
              { origins: [center], destinations: chunks[ci], travelMode: window.google.maps.TravelMode.DRIVING },
              (response: any, status: string) => {
                console.log(`[Sustenta] DistanceMatrix chunk ${ci} → status=${status}`);
    ...
    console.log('[Sustenta] Lista final:', lista.length, lista.map(e => e.name));

    // 6. Salva e renderiza marcadores
    setEcopontos(lista);
  ...
  const abrirEcoponto = React.useCallback((place_id: string) => {
    ...
    dirService.route(
      { origin: { lat: pos.lat, lng: pos.lng }, destination: { lat: eco.lat, lng: eco.lng }, travelMode: window.google.maps.TravelMode.DRIVING },
      (result: any, status: string) => {
        if (status === 'OK') {
          direcaoRef.current.setDirections(result);
          const leg = result.routes[0].legs[0];
          setRota({ distancia: leg.distance.text, duracao: leg.duration.text, passos: leg.steps.slice(0, 6).map((s: any) => s.instructions.replace(/<[^>]+>/g, '')) });
        }
  ...
  return (
    <div className="relative w-full" style={{ height: 'calc(100vh - 120px)', minHeight: 600 }}>
      ...
      <div className="absolute top-0 left-0 right-0 flex items-center justify-between px-5 py-4" style={{ zIndex: 2 }}>
        <div>
          <h1 className="text-xl font-black text-white">Mapa Sustentável</h1>
          <p className="text-xs font-medium mt-0.5" style={{ color: '#34d399' }}>
            {loadingEco ? 'Buscando ecopontos...' :
             ...
             'Explore o índice de sustentabilidade por estado'}
          </p>
        </div>
                      ...
                      {!estadoAtivoInfo ? (
                        ...
                        <div className="space-y-4">
                          ...
                          <div className="grid grid-cols-2 gap-2">
                            {[
                              { label: 'Usuários', value: estadoAtivoInfo.totalUsuarios.toLocaleString('pt-BR') },
                              { label: 'Média/mês', value: `R$ ${estadoAtivoInfo.mediaConsumo}` },
                              { label: 'Seu consumo', value: meuConsumo > 0 ? `R$ ${Math.round(meuConsumo)}` : '—' },
                              { label: 'vs. média', value: meuConsumo > 0 ? `${meuConsumo < estadoAtivoInfo.mediaConsumo ? '▼' : '▲'} ${Math.abs(Math.round(((meuConsumo - estadoAtivoInfo.mediaConsumo) / estadoAtivoInfo.mediaConsumo) * 100))}%` : '—' },
                            ].map(item => (
                          ...
                          {meuConsumo > 0 && meuConsumo < estadoAtivoInfo.mediaConsumo && (
                            <div className="flex items-start gap-2 p-3 rounded-xl" style={{ background: 'rgba(16,185,129,0.1)', border: '1px solid rgba(16,185,129,0.2)' }}>
                              ...
                              <p className="text-xs text-emerald-300 font-medium">Você consome menos que a média deste estado!</p>
                            </div>
                          )}
                        </div>
                      )}
                    ...
                    <motion.div key="ecopontos" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex flex-col">
                      {!userPos && !loadingEco ? (
                        <div className="flex flex-col items-center justify-center py-10 text-center gap-3 px-4">
                          ...
                          <p className="text-sm font-bold text-slate-400">Nenhuma localização</p>
                              ...
                              <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }}
                                ...
                                <div className="p-4" style={{ background: 'rgba(16,185,129,0.07)' }}>
                                  ...
                                  {loadingRota ? (
                                    ...
                                    <p className="text-xs text-slate-400">Não foi possível calcular a rota.</p>
                                  )}
                                </div>
                              </motion.div>
                            )}
                          ...
                          <button key={est.sigla} onClick={() => { setEstadoAtivo(est.sigla); setAbaPainel('estado'); }}
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-1.5 mb-1">
                                ...
                                {est.sigla === meuEstado && <span className="text-[9px] font-bold px-1.5 py-0.5 rounded-full shrink-0" style={{ background: 'rgba(13,148,136,0.2)', border: '1px solid rgba(13,148,136,0.3)', color: '#2dd4bf' }}>Você</span>}
                              </div>
                              <div className="flex items-center gap-2">
