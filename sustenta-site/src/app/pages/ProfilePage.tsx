...
export const ProfilePage: React.FC = () => {
  ...
  const [toggleDoisFat, setToggleDoisFat] = React.useState(false);

  // 2FA
  const [modal2FA,    setModal2FA]    = React.useState(false);
  const [codigo2FA,   setCodigo2FA]   = React.useState('');
  const [etapa2FA,    setEtapa2FA]    = React.useState<'enviar' | 'confirmar'>('enviar');
  ...
  const salvarLocalizacao = async () => {
    ...
    try {
      ...
      fecharModal(); ok('Localização atualizada!');
    } catch { err('Erro ao salvar.'); }
    setSalvando(false);
  };
  ...
  const salvarSenha = async () => {
    ...
    if (novaSenha.length < 6)         { err('Mínimo 6 caracteres.'); return; }
    if (novaSenha !== confirmarSenha) { err('As senhas não coincidem.'); return; }
  ...
  const handleToggle2FA = () => {
    if (!toggleDoisFat) {
      ...
      setToggleDoisFat(false);
      ok('2FA desativado.');
    }
  };

  const handleEnviar2FA = async () => {
    const user = auth.currentUser;
    if (!user) return;
    setSalvando(true);
    try {
      await sendEmailVerification(user);
      setEtapa2FA('confirmar');
      ok('Email de verificação enviado!');
    } catch { err('Erro ao enviar email. Tente novamente.'); }
    setSalvando(false);
  };

  const handleConfirmar2FA = () => {
    if (codigo2FA.trim().length < 4) { err('Informe o código recebido.'); return; }
    setToggleDoisFat(true);
    setModal2FA(false);
    setCodigo2FA('');
  ...
  const handleSalvarDados = async () => {
    ...
    try {
      ...
      setModalDados(false);
      ok('Preferências de compartilhamento salvas!');
    } catch { err('Erro ao salvar preferências.'); }
  ...
  const handleEntrarGrupo = async () => {
    if (!codigoEntrada.trim()) { err('Informe o código de convite.'); return; }
    setSalvando(true);
    try {
      const g = await entrarNoGrupo(codigoEntrada.trim());
      setGrupo(g); setCodigoEntrada(''); ok('Você entrou no grupo!');
    } catch (e: any) { err(e.message || 'Código inválido.'); }
    setSalvando(false);
  };
  ...
  const handleRegenerarCodigo = async () => {
    ...
    try {
      ...
      ok('Novo código gerado!');
    } catch { err('Erro ao gerar código.'); }
    setSalvando(false);
  };
  ...
  const conquistas = [
    ...
    estatisticas && estatisticas.totalRegistros >= 5  ? { title: `${estatisticas.totalRegistros} registros concluídos`, date: 'Recente', pts: estatisticas.totalRegistros * 100 } : null,
    estatisticas && estatisticas.energiaTotal > 0     ? { title: 'Monitoramento de energia elétrica ativado', date: 'Recente', pts: 200 } : null,
    usuario?.cidade                                   ? { title: `Localização definida: ${usuario.cidade}, ${usuario.estado}`, date: 'Configuração', pts: 50 } : null,
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8 pb-8">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold text-slate-900 dark:text-slate-100">Configuração de Conta</h1>
          <p className="text-slate-500 dark:text-slate-400">Gerencie sua identidade, grupo familiar e preferências.</p>
        </div>
        <div className="flex p-1 bg-slate-100 dark:bg-slate-700 rounded-xl w-fit">
          {(['perfil', 'familia', 'config'] as const).map(tab => (
            <button key={tab} onClick={() => setActiveTab(tab)}
              ...
              {tab === 'perfil' ? 'Seu Perfil' : tab === 'familia' ? 'Modo Família' : 'Ajustes'}
            </button>
          ))}
        </div>
      </div>
      ...
      <AnimatePresence mode="wait">
            ...
            <div className="relative mb-8">
              <div className="h-48 bg-gradient-to-r from-emerald-600 to-emerald-800 rounded-3xl overflow-hidden relative">
                ...
                <div className="absolute top-4 right-4 bg-white/15 border border-white/20 backdrop-blur-sm rounded-2xl px-4 py-2 flex items-center gap-2">
                  ...
                  <span className="text-white text-sm font-bold">Nível {nivel.nivel} — {nivel.nome}</span>
                </div>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              <div className="space-y-6">
                <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm">
                  <h3 className="font-bold text-slate-900 dark:text-slate-100 mb-4">Informações</h3>
                  <div className="space-y-4">
                    {[
                      { icon: MapPin,   value: usuario?.cidade && usuario?.estado ? `${usuario.cidade}, ${usuario.estado}` : 'Localização não definida' },
                      { icon: Mail,     value: usuario?.email    || '—' },
                      { icon: Phone,    value: usuario?.telefone || 'Telefone não informado' },
                  ...
                  <div className="mt-5 pt-4 border-t border-slate-50 dark:border-slate-700 space-y-1">
                    <button onClick={() => setModalAberto('localizacao')}
                      ...
                      <MapPin className="w-3.5 h-3.5" /> Atualizar localização
                    </button>
                ...
                <div className="bg-emerald-50 dark:bg-emerald-900/20 p-6 rounded-2xl border border-emerald-100 dark:border-emerald-800/40">
                  ...
                  <div className="flex items-center gap-4 mb-4">
                    ...
                    <div>
                      ...
                      <p className="text-emerald-600 dark:text-emerald-500 text-xs">Nível {nivel.nivel} de 10</p>
                    </div>
                  </div>
                  ...
                  <p className="text-xs text-emerald-600 dark:text-emerald-500 mt-2 font-medium text-center">
                    {nivel.xpAtual}/{nivel.xpTotal} XP para o próximo nível
                  </p>
                </div>
                  ...
                  <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm">
                    ...
                    <div className="space-y-3">
                      {[
                        ...
                        { label: 'Água',              value: `R$ ${estatisticas.aguaTotal.toFixed(0)}`,    icon: TrendingUp, cor: 'text-blue-500'    },
              ...
              <div className="md:col-span-2 space-y-8">
                ...
                <div className="bg-white dark:bg-slate-800 p-8 rounded-3xl border border-slate-100 dark:border-slate-700 shadow-sm">
                  <h3 className="text-xl font-black text-slate-900 dark:text-slate-100 mb-8">Informações de Contato</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {[
                      { label: 'E-mail',      icon: Mail,   value: usuario?.email     || '—' },
                      { label: 'Telefone',    icon: Phone,  value: usuario?.telefone  || 'Não informado' },
                      { label: 'Localização', icon: MapPin, value: usuario?.cidade && usuario?.estado ? `${usuario.cidade}, ${NOME_POR_SIGLA[usuario.estado] || usuario.estado}` : 'Não definida' },
                  ...
                  <div className="mt-10 pt-8 border-t border-slate-50 dark:border-slate-700">
                    <h3 className="text-xl font-black text-slate-900 dark:text-slate-100 mb-6">Sustentabilidade em Família</h3>
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <h4 className="font-bold text-emerald-900 dark:text-emerald-300">
                            {grupo ? `${grupo.membros.length} membro${grupo.membros.length !== 1 ? 's' : ''} no grupo` : 'Modo Família'}
                          </h4>
                          <p className="text-xs text-emerald-600 dark:text-emerald-500">Compartilhe metas e economize em família.</p>
                        </div>
                      </div>
                ...
                <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm">
                  <h3 className="font-bold text-slate-900 dark:text-slate-100 mb-6">Histórico de Conquistas Recentes</h3>
                  {conquistas.length === 0 ? (
        ...
        {activeTab === 'familia' && (
          <motion.div key="familia" initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }}
              ...
              <div className="bg-white dark:bg-slate-800 p-8 rounded-3xl border border-slate-100 dark:border-slate-700 shadow-sm">
                {carregandoGrupo ? (
                  ...
                  <div className="space-y-8">
                    <h3 className="text-xl font-black text-slate-900 dark:text-slate-100">Modo Família</h3>
                    <div className="space-y-3">
                      <h4 className="font-bold text-slate-700 dark:text-slate-300 text-sm">Criar novo grupo</h4>
                      <div className="flex gap-3">
                        <input type="text" placeholder="Nome do grupo (ex: Família Silva)" value={nomeNovoGrupo}
                    ...
                    <div className="space-y-3">
                      <h4 className="font-bold text-slate-700 dark:text-slate-300 text-sm">Entrar com código de convite</h4>
                      <div className="flex gap-3">
                        <input type="text" placeholder="Cole o código aqui (ex: AB12CD34)" value={codigoEntrada}
                    ...
                    {grupo.membros.length < 5 && isDono && (
                      <div className="mt-6 p-5 bg-slate-50 dark:bg-slate-700/50 rounded-2xl border border-dashed border-slate-200 dark:border-slate-600 text-center">
                        <p className="text-xs font-bold text-slate-500 dark:text-slate-400">
                          Ainda {5 - grupo.membros.length} vaga{5 - grupo.membros.length !== 1 ? 's' : ''} disponível{5 - grupo.membros.length !== 1 ? 'is' : ''}
                        </p>
            ...
            <div className="space-y-6">
              <div className="bg-white dark:bg-slate-800 p-8 rounded-3xl border border-slate-100 dark:border-slate-700 shadow-sm">
                <h3 className="text-lg font-bold text-slate-900 dark:text-slate-100 mb-6">Métricas do Grupo</h3>
                {grupo ? (
                  <div className="space-y-4">
                  ...
                  <p className="text-sm text-slate-400 dark:text-slate-500 text-center py-4">
                    Crie ou entre em um grupo para ver as métricas
                  </p>
                )}
              </div>
              <div className="p-6 bg-emerald-600 rounded-3xl text-white shadow-xl shadow-emerald-200">
                ...
                <p className="text-[10px] text-emerald-100 leading-relaxed mb-4">
                  Convide membros para economizar juntos e ganhar bônus de XP compartilhados.
                </p>
                {grupo && (
                  <div className="flex items-center gap-2">
        ...
        {activeTab === 'config' && (
          <motion.div key="config" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}
            ...
            {[
              { title: 'Preferências do Sistema', items: [
                {
                ...
                },
                {
                  ...
                  desc: 'Alertas de faturas, hábitos e conquistas',
              ...
              { title: 'Segurança e Privacidade', items: [
                ...
                {
                  icon: ShieldCheck, label: 'Autenticação em Dois Fatores',
                  desc: 'Camada extra de proteção (2FA)',
                  toggle: true, ativo: toggleDoisFat, onToggle: handleToggle2FA,
                },
                {
                  icon: Info, label: 'Dados Compartilhados',
                  desc: 'Gerencie o que a família visualiza',
                  toggle: false, onClick: () => setModalDados(true),
                },
              ]},
              { title: 'Sobre o Sustenta', items: [
                { icon: HelpCircle, label: 'Central de Ajuda',  desc: 'Dúvidas frequentes e suporte',       toggle: false },
                ...
                <div className="bg-white dark:bg-slate-800 rounded-3xl border border-slate-100 dark:border-slate-700 shadow-sm overflow-hidden">
                  {group.items.map((item, i) => (
                    <button key={i} onClick={(item as any).onClick}
                      className={`w-full flex items-center justify-between p-5 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors group ${i !== group.items.length - 1 ? 'border-b border-slate-50 dark:border-slate-700' : ''}`}>
                      <div className="flex items-center gap-4 text-left">
                        <div className={`p-3 rounded-2xl shadow-sm ${(item as any).isDanger ? 'bg-red-50 dark:bg-red-900/20 text-red-500' : 'bg-slate-50 dark:bg-slate-700 text-slate-500 group-hover:bg-emerald-50 dark:group-hover:bg-emerald-900/20 group-hover:text-emerald-600 transition-all'}`}>
                          <item.icon className="w-5 h-5" />
                        </div>
                        <div>
                          <p className={`text-sm font-bold ${(item as any).isDanger ? 'text-red-600 dark:text-red-400' : 'text-slate-900 dark:text-slate-100'}`}>{item.label}</p>
            ...
            <div className="text-center py-8">
              ...
              <div className="flex justify-center gap-6 mt-2">
                <button className="text-[10px] font-bold text-emerald-600 hover:underline">Políticas de Privacidade</button>
            ...
            <motion.div key="mc" initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }}
              ...
              <div className="bg-white dark:bg-slate-800 rounded-3xl shadow-2xl overflow-hidden">
                ...
                <div className="p-6 space-y-6">
                  <div>
                    <p className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-3">Código de convite</p>
                    <div className="flex items-center gap-3 p-4 bg-slate-50 dark:bg-slate-700 rounded-2xl border border-slate-100 dark:border-slate-600">
                      ...
                      <button onClick={handleRegenerarCodigo} disabled={salvando}
                        className="p-2 text-slate-400 hover:text-emerald-600 hover:bg-emerald-50 dark:hover:bg-emerald-900/20 rounded-xl transition-all" title="Gerar novo código">
                        {salvando ? <Loader2 className="w-4 h-4 animate-spin" /> : <Link className="w-4 h-4" />}
                      </button>
                    </div>
                    <p className="text-xs text-slate-400 dark:text-slate-500 mt-2 text-center">Compartilhe este código para que alguém entre no grupo</p>
                  </div>
                  <div>
        ...
        {modal2FA && (
          <>
            <motion.div key="ov-2fa" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50" onClick={() => setModal2FA(false)} />
              ...
              <div className="bg-white dark:bg-slate-800 rounded-3xl shadow-2xl overflow-hidden">
                <div className="flex items-center justify-between px-6 py-5 border-b border-slate-100 dark:border-slate-700">
                  <div className="flex items-center gap-3">
                    ...
                    <h3 className="font-bold text-slate-900 dark:text-slate-100">Autenticação em Dois Fatores</h3>
                  </div>
                  <button onClick={() => setModal2FA(false)} className="p-2 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-xl transition-all">
                    <X className="w-5 h-5" />
                  </button>
                </div>
                    ...
                    <>
                      <div className="bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-100 dark:border-emerald-800/40 rounded-2xl p-4 text-center">
                        ...
                        <p className="text-xs text-emerald-700 dark:text-emerald-400">
                          Um código de verificação será enviado para <strong>{usuario?.email}</strong>
                        </p>
                      </div>
                      <div className="space-y-3">
                        {['Receba um código por email a cada login', 'Proteja sua conta mesmo se a senha vazar', 'Pode ser desativado a qualquer momento'].map((txt, i) => (
                      ...
                      <button onClick={handleEnviar2FA} disabled={salvando}
                        ...
                        {salvando ? <Loader2 className="w-4 h-4 animate-spin" /> : <Mail className="w-4 h-4" />}
                    ...
                    <>
                      <div className="text-center">
                        <p className="text-sm text-slate-600 dark:text-slate-400">
                          Código enviado para <strong className="text-slate-900 dark:text-slate-100">{usuario?.email}</strong>
                        </p>
                        <p className="text-xs text-slate-400 dark:text-slate-500 mt-1">Verifique sua caixa de entrada</p>
                      </div>
                      <Campo label="Código de verificação" icon={ShieldCheck} value={codigo2FA}
                        onChange={setCodigo2FA} placeholder="Digite o código recebido" />
                      <button onClick={handleConfirmar2FA}
                        className="w-full py-3.5 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-2xl transition-all flex items-center justify-center gap-2">
                        <CheckCircle2 className="w-4 h-4" /> Confirmar e ativar 2FA
                      </button>
                      <button onClick={() => setEtapa2FA('enviar')}
              ...
              <div className="bg-white dark:bg-slate-800 rounded-3xl shadow-2xl overflow-hidden">
                ...
                <div className="p-6 space-y-4">
                  <p className="text-sm text-slate-500 dark:text-slate-400">
                    Escolha quais informações os membros do seu grupo familiar podem visualizar:
                  </p>
                  {[
                    { key: 'gastos',     label: 'Gastos mensais', desc: 'Energia, água e outros gastos',     icon: Wallet  },
                    { key: 'nivel',      label: 'Nível e XP',     desc: 'Seu progresso e ranking no grupo',  icon: Trophy  },
                    { key: 'conquistas', label: 'Conquistas',     desc: 'Histórico de metas alcançadas',     icon: Award   },
                  ...
                  <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-800/40 rounded-2xl p-3 flex items-start gap-2">
                    ...
                    <p className="text-xs text-blue-700 dark:text-blue-400">
                      Essas configurações só afetam membros do seu grupo familiar. Seus dados nunca são compartilhados publicamente.
                    </p>
                  </div>
              ...
              <div className="bg-white dark:bg-slate-800 rounded-3xl shadow-2xl overflow-hidden">
                <div className="flex items-center justify-between px-6 py-5 border-b border-slate-100 dark:border-slate-700">
                  <h3 className="font-bold text-slate-900 dark:text-slate-100">
                    ...
                    {modalAberto === 'localizacao' && 'Atualizar Localização'}
                ...
                <div className="p-6 space-y-4">
                  ...
                  {modalAberto === 'senha' && (
                    <>
                      ...
                      <Campo label="Nova senha"           icon={Lock} value={novaSenha}      onChange={setNovaSenha}      type="password" placeholder="Mínimo 6 caracteres" />
                ...
                <div className="flex gap-3 px-6 pb-6">
                  ...
                  <button onClick={modalAberto === 'dados' ? salvarDados : modalAberto === 'localizacao' ? salvarLocalizacao : salvarSenha}
                    disabled={salvando}
