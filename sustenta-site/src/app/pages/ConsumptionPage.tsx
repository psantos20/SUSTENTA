...
import { collection, addDoc, Timestamp, query, where, getDocs, deleteDoc, doc, updateDoc, orderBy } from 'firebase/firestore';
...
export const ConsumptionPage: React.FC = () => {
  ...
  const badgeColor: Record<string, string> = {
    ...
    outro: 'bg-indigo-100 text-indigo-700',
  };
  ...
  return (
    <div className="space-y-8 max-w-2xl mx-auto">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white rounded-2xl border border-slate-100 shadow-sm p-8">
        ...
        <div className="grid grid-cols-3 gap-3 mb-6">
          {[
            ...
            return (
              <button key={cat.id} onClick={() => { setCategoria(cat.id); setSubcategoria(''); setSubcategoriaCustom(''); }}
                className={`flex flex-col items-center gap-2 p-4 rounded-xl border-2 transition-all ${isActive ? 'border-emerald-500 bg-emerald-50 text-emerald-700' : 'border-slate-100 hover:border-slate-200 text-slate-500'}`}>
        ...
        {/* Dropdown de subcategoria para Outros */}
        <AnimatePresence>
          {categoria === 'outro' && (
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="mb-6 overflow-hidden">
              <label className="text-sm font-medium text-slate-700 block mb-2">Tipo de gasto</label>
              <div className="relative">
                <select value={subcategoria} onChange={e => { setSubcategoria(e.target.value); setSubcategoriaCustom(''); }}
                  className="w-full px-4 py-3 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500 text-slate-900 appearance-none bg-white pr-10">
                ...
                {subcategoria === 'Outro' && (
                  <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="mt-3 overflow-hidden">
                    <input type="text" value={subcategoriaCustom} onChange={e => setSubcategoriaCustom(e.target.value)}
                      className="w-full px-4 py-3 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500 text-slate-900"
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input type="number" min="0" step="0.01" required value={valor} onChange={e => setValor(e.target.value)}
                className="w-full pl-12 pr-4 py-3 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500 text-slate-900 text-lg font-bold"
                placeholder="0,00" />
            </div>
          </div>
          ...
          <div>
            ...
            <div className="relative">
              <div className="w-full px-4 py-3 border border-slate-200 rounded-xl text-slate-900 bg-white">
                {mesLabel(mes)}
              </div>
      ...
      {/* Histórico */}
      <div>
        <h2 className="text-lg font-bold text-slate-900 mb-4">Histórico de Registros</h2>
        {carregando ? (
          <div className="flex justify-center py-8"><Loader2 className="w-6 h-6 text-emerald-500 animate-spin" /></div>
        ) : registros.length === 0 ? (
          <div className="bg-white rounded-2xl border border-slate-100 p-8 text-center text-slate-400">Nenhum registro ainda.</div>
        ) : (
            ...
            {registros.map(r => {
              ...
              return (
                <motion.div key={r.id} layout initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
                  ...
                  <div className={`p-2.5 rounded-xl ${badgeColor[r.categoria] ?? 'bg-slate-100 text-slate-600'}`}>
                    ...
                    {isEditando ? (
                      <div className="flex gap-2 flex-wrap">
                        <input type="number" value={editValor} onChange={e => setEditValor(e.target.value)}
                          className="border border-slate-200 rounded-lg px-2 py-1 text-sm w-24 focus:outline-none focus:ring-2 focus:ring-emerald-500" />
                        <input type="text" value={editDescricao} onChange={e => setEditDescricao(e.target.value)}
                          className="border border-slate-200 rounded-lg px-2 py-1 text-sm flex-1 focus:outline-none focus:ring-2 focus:ring-emerald-500" placeholder="Descrição" />
                      </div>
                    ) : (
                      <>
                        <p className="text-sm font-bold text-slate-800">
                          {registroLabel(r)} — <span className="text-emerald-600">R$ {r.valor.toFixed(2)}</span>
                        </p>
                        <p className="text-xs text-slate-400">{mesLabel(r.mes)}{r.descricao ? ` • ${r.descricao}` : ''}</p>
                      </>
                    )}
                  </div>
                  <div className="flex items-center gap-2">
                    {isEditando ? (
                      <>
                        <button onClick={() => handleSaveEdit(r.id)} className="p-2 text-emerald-600 hover:bg-emerald-50 rounded-lg"><Check className="w-4 h-4" /></button>
                        <button onClick={() => setEditandoId(null)} className="p-2 text-slate-400 hover:bg-slate-50 rounded-lg"><X className="w-4 h-4" /></button>
                      </>
                    ) : (
                      <>
                        <button onClick={() => handleEdit(r)} className="p-2 text-slate-400 hover:text-emerald-600 hover:bg-emerald-50 rounded-lg"><Pencil className="w-4 h-4" /></button>
                        <button onClick={() => handleDelete(r.id)} className="p-2 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-lg"><Trash2 className="w-4 h-4" /></button>
                      </>
                    )}
                  </div>
                </motion.div>
