...
const PALAVRAS_SUSTENTABILIDADE = [
  'sustentabilidade', 'sustentavel', 'meio ambiente', 'ecologia', 'ecologico',
  'energia solar', 'energia renovavel', 'energia eolica', 'energia limpa',
  'biomassa', 'biocombustivel', 'hidrogenio verde',
  'reciclagem', 'reciclar', 'residuos', 'lixo eletronico', 'coleta seletiva',
  'desmatamento', 'reflorestamento', 'biodiversidade', 'conservacao ambiental',
  'aquecimento global', 'mudancas climaticas', 'mudanca climatica', 'emissao de carbono',
  'pegada de carbono', 'co2', 'gases de efeito estufa',
  'saneamento', 'crise hidrica', 'escassez de agua', 'desperdicio de agua',
  'ecoponto', 'economia circular', 'impacto ambiental',
  'ibama', 'inpe', 'amazonia', 'cerrado', 'mata atlantica', 'pantanal',
  'cop', 'acordo de paris', 'protocolo de kyoto',
  'carbon', 'esg', 'green', 'emisso', 'poluicao',
];
...
function isArtigoSustentavel(titulo: string, descricao: string): boolean {
  const texto = normalizarTexto(`${titulo} ${descricao}`);

  return PALAVRAS_SUSTENTABILIDADE.some((p) =>
    texto.includes(normalizarTexto(p))
  );
}
...
const NOTICIAS_CATEGORIAS = [
  {
    ...
    query: '"sustentabilidade" OR "meio ambiente" OR "impacto ambiental" OR "ecologia"',
    icon: Leaf,
  ...
  },
  {
    id: 'energia',
    label: 'Energia',
    query: '"energia solar" OR "energia renovavel" OR "energia eolica" OR "energia limpa" OR "painel solar" OR "biocombustivel"',
    icon: Zap,
  ...
  },
  {
    id: 'agua',
    label: 'Agua',
    query: '"crise hidrica" OR "saneamento basico" OR "escassez de agua" OR "desperdicio de agua" OR "tratamento de esgoto"',
    icon: Droplets,
  ...
  },
  {
    id: 'clima',
    label: 'Clima',
    query: '"mudancas climaticas" OR "aquecimento global" OR "desmatamento" OR "emissao de carbono" OR "gases de efeito estufa" OR "COP" OR "acordo de paris"',
    icon: Wind,
  ...
  },
  {
    id: 'residuos',
    label: 'Residuos',
    query: '"reciclagem" OR "residuos solidos" OR "economia circular" OR "coleta seletiva" OR "lixo eletronico" OR "compostagem"',
    icon: Globe,
...
async function salvarNoticiaCache(categoriaId: string, noticias: Noticia[]) {
  try {
    await setDoc(doc(db, 'noticias_cache', categoriaId), {
      noticias,
      cachadoEm: Date.now(),
    });
  } catch {}
...
async function buscarNoticias(categoria: typeof NOTICIAS_CATEGORIAS[0]): Promise<Noticia[]> {
  ...
  const noticias: Noticia[] = (data.articles ?? [])
    .filter((a: any) => {
      ...
      return isArtigoSustentavel(a.title, a.description ?? '');
    })
    .slice(0, 8)
    .map((a: any, i: number) => ({
      id: `${categoria.id}-${i}`,
...
function calcularCarbono(registros: Registro[]) {
  const energiaKg = registros
    .filter((r) => r.categoria === 'energia')
    .reduce((s, r) => s + r.valor * CARBON_FACTORS.energia, 0);

  const aguaKg = registros
    .filter((r) => r.categoria === 'agua')
    .reduce((s, r) => s + r.valor * CARBON_FACTORS.agua, 0);

  const outroKg = registros
    .filter((r) => r.categoria === 'outro')
    .reduce((s, r) => s + r.valor * CARBON_FACTORS.outro, 0);
  ...
  const res = await fetch('https://api.anthropic.com/v1/messages', {
    ...
    body: JSON.stringify({
      ...
      messages: [
        {
          role: 'user',
          content: `Consultor de sustentabilidade brasileiro. Dados mensais: Energia R$${e.toFixed(2)}, Agua R$${a.toFixed(2)}, Outros R$${o.toFixed(2)}, CO2 total ${kg.toFixed(1)}kg, maior gasto: ${maior}. Escreva analise personalizada de 3-4 frases em portugues brasileiro com: avaliacao do nivel atual, acao concreta para a categoria principal e impacto estimado com numeros.`,
        },
      ],
    }),
  });
...
async function salvarArtigoLido(uid: string, artigoId: string) {
  const snap = await getDoc(doc(db, 'usuarios', uid));
  const atual = snap.data()?.educacao?.xpEducacao ?? 0;

  await updateDoc(doc(db, 'usuarios', uid), {
    'educacao.artigosLidos': arrayUnion(artigoId),
    'educacao.xpEducacao': atual + 50,
  });
}

async function salvarQuizCompleto(
  uid: string,
  catId: string,
  acertos: number,
  total: number,
  ...
  const xpEducacaoAtual = userSnap.data()?.educacao?.xpEducacao ?? 0;

  await updateDoc(doc(db, 'usuarios', uid), {
    'educacao.quizzesCompletos': arrayUnion(entry),
    'educacao.xpEducacao': xpEducacaoAtual + xpGanho,
  });
...
async function salvarDesafioAceito(uid: string, desafioId: string, xp: number, nomeUsuario: string) {
  ...
  await updateDoc(doc(db, 'usuarios', uid), {
    ...
    'educacao.xpEducacao': xpEducacaoAtual + xp,
  });
...
function GlassCard({
  ...
  return (
    <motion.div
      ...
      className={clsx(
        'rounded-2xl border backdrop-blur-sm transition-colors bg-white/5 border-white/10 hover:border-white/20',
        className
      )}
    >
      {children}
...
function DesafioCard({
  ...
  onAceitarDesafio: (desafio: typeof DESAFIOS_SEMANA[0]) => Promise<void>;
}) {
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-start gap-3 flex-1 min-w-0">
          ...
          <div>
            ...
            <p className="font-bold text-white text-sm">
              {desafio.titulo.replace('Desafio do Dia: ', '')}
            </p>
        ...
        <div className="flex flex-col items-end gap-2 flex-shrink-0">
          ...
          <motion.button
            ...
            disabled={jaAceito || aceitando}
            className={clsx(
              'flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-bold transition-all',
              jaAceito
                ? 'bg-emerald-600/30 text-emerald-300 border border-emerald-600/40 cursor-default'
      ...
      <div className="p-4 space-y-2">
        ...
        <div className="flex items-center justify-between pt-1">
          ...
          >
            ...
            <ChevronDown
              size={11}
              className={clsx('transition-transform', expandida && 'rotate-180')}
            />
          </button>
...
function ArtigosTab({
  ...
  progress: UserProgress | null;
  onArtigoLido: (id: string) => Promise<void>;
}) {
  ...
  async function carregar(cat: typeof NOTICIAS_CATEGORIAS[0], forceRefresh = false) {
    ...
    try {
      if (forceRefresh) {
        try {
          await setDoc(doc(db, 'noticias_cache', cat.id), {
            noticias: [],
            cachadoEm: 0,
          });
        } catch {}
      ...
      if (progress && !progress.artigosLidos.includes(`news-${cat.id}`)) {
        await onArtigoLido(`news-${cat.id}`);
      }
    } catch (e: any) {
      setErro(
        e?.message ||
          'Nao foi possivel carregar as noticias. Verifique sua conexao e tente novamente.'
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-start gap-2 px-4 py-3 rounded-xl bg-emerald-600/10 border border-emerald-500/20">
        <Leaf size={13} className="text-emerald-400 flex-shrink-0 mt-0.5" />

        <p className="text-xs text-emerald-300 leading-relaxed">
          Noticias reais sobre sustentabilidade, meio ambiente e consumo consciente via
          NewsAPI. Filtradas automaticamente para exibir apenas conteudo relevante ao
          tema.
        </p>
      </div>
      ...
      <div className="flex items-center justify-between">
        ...
        <button
          onClick={() => carregar(catAtiva, true)}
          disabled={loading}
      ...
      {!loading && !erro && noticiasFiltradas.length > 0 && (
        <>
          <p className="text-xs text-slate-500">
            ...
            {noticiasFiltradas.length !== 1 ? 's' : ''} sobre{' '}
            {catAtiva.label.toLowerCase()}
          </p>
      ...
      {!loading && !erro && noticiasFiltradas.length === 0 && noticias.length > 0 && (
        <GlassCard className="p-10 text-center">
          <p className="text-slate-400 text-sm">
            Nenhuma noticia encontrada para "{busca}".
          </p>
        </GlassCard>
      )}
      ...
      {!loading && !erro && noticias.length === 0 && (
        <GlassCard className="p-10 text-center space-y-3">
          <Leaf size={32} className="text-emerald-700 mx-auto" />

          <p className="text-slate-300 text-sm font-semibold">
            Nenhuma noticia de sustentabilidade encontrada
          </p>

          <p className="text-slate-500 text-xs leading-relaxed max-w-xs mx-auto">
            A NewsAPI nao retornou artigos relevantes para esta categoria no momento.
            Tente atualizar ou escolha outra categoria.
          </p>

          <button
            onClick={() => carregar(catAtiva, true)}
            className="flex items-center gap-2 px-4 py-2 rounded-xl bg-emerald-600/20 border border-emerald-600/30 text-emerald-300 text-xs font-semibold transition-all mx-auto hover:bg-emerald-600/30"
          >
            <RotateCcw size={12} /> Tentar novamente
          </button>
        </GlassCard>
...
function VideosTab() {
  ...
  const videosFiltrados = VIDEOS.filter(
    (v) => tagAtiva === 'Todos' || v.tag === tagAtiva
  );
          ...
          >
            {playing === v.id ? (
              ...
              >
                ...
                <div className="absolute bottom-2.5 right-2.5 bg-black/70 text-white text-[10px] font-bold px-1.5 py-0.5 rounded backdrop-blur-sm">
                  {v.duracao}
                </div>

                <div className="absolute top-2.5 left-2.5 bg-black/50 text-white text-[10px] font-bold px-2 py-0.5 rounded-full backdrop-blur-sm border border-white/20">
                  {v.tag}
                </div>
              </div>
            )}
...
function QuizTab({
  ...
  progress: UserProgress | null;
  onQuizCompleto: (catId: string, acertos: number, total: number, xp: number) => Promise<void>;
}) {
  ...
  async function proxima() {
    ...
    if (qIdx + 1 >= cat.perguntas.length) {
      setFinalizado(true);
      await onQuizCompleto(catSel!, totalAcertos, cat.perguntas.length, xpGanho);
      setLoadingRanking(true);

      carregarRanking().then((r) => {
        setRanking(r);
  ...
  if (!catSel) {
    return (
      <div className="space-y-4">
        <p className="text-slate-400 text-sm">
          Escolha uma categoria. Cada pergunta tem {QUIZ_TEMPO} segundos!
        </p>
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-400 mt-0.5">
                    {c.perguntas.length} perguntas · {QUIZ_TEMPO}s cada
                  </p>
                  {completou && (
                    <p className="text-[10px] text-emerald-400 mt-0.5">Completado!</p>
                  )}
                </div>

                <ChevronRight
                  size={16}
                  className="text-slate-500 ml-auto group-hover:translate-x-1 transition-transform"
                />
              </motion.button>
            );
          })}
        ...
        <GlassCard className="p-8 text-center space-y-5 relative overflow-hidden">
          ...
          <div>
            <h3 className="text-3xl font-black text-white">
              {totalAcertos}/{cat.perguntas.length}
            </h3>
                ...
                return (
                  ...
                  >
                    ...
                    <span
                      className={clsx(
                        'text-sm flex-1',
                        isMe ? 'text-emerald-300 font-bold' : 'text-slate-300'
                      )}
                    >
                      {r.nome}
                      {isMe ? ' (voce)' : ''}
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-bold tabular-nums" style={{ color: tempoColor }}>
            {tempo}s
          </span>
        </div>
      </div>
        ...
        >
          <GlassCard className="p-5 space-y-4 relative overflow-hidden">
                ...
                return (
                  <motion.button
                    ...
                    className={clsx(
                      ...
                      !resp &&
                        'hover:border-emerald-500/60 hover:bg-emerald-600/10 cursor-pointer',
            ...
            <AnimatePresence>
              {selecionada !== null && (
                <motion.div
                  ...
                  className={clsx(
                    'rounded-xl p-4 border text-xs leading-relaxed overflow-hidden',
            ...
            {selecionada !== null && (
              ...
              >
                {qIdx + 1 >= cat.perguntas.length ? (
                  <>
                    <Trophy size={14} /> Ver resultado
                  </>
                ) : (
                  <>
                    Proxima <ArrowRight size={14} />
                  </>
                )}
              </motion.button>
            )}
  ...
  async function calcular() {
    ...
    try {
      const snap = await getDocs(
        query(collection(db, 'registros'), where('uid', '==', uid), where('mes', '==', mesSel))
      );
      ...
      const hist = await Promise.all(
        meses6.slice().reverse().map(async (m) => {
          const s = await getDocs(
            query(collection(db, 'registros'), where('uid', '==', uid), where('mes', '==', m))
          );
      ...
      try {
        ...
        setDicaIA('Continue reduzindo seu consumo e acompanhe sua evolucao mensal.');
      }
    } finally {
      setLoading(false);
      setLoadingIA(false);
        ...
        <GlassCard className="flex flex-col items-center justify-center py-16 gap-4 text-center">
          ...
          <div>
            ...
            <p className="text-sm mt-1 text-slate-400">
              Selecione um mes e clique em{' '}
              <span className="text-emerald-400">Calcular pegada</span>.
            </p>
          </div>
        </GlassCard>
      )}
          ...
          >
            <GlassCard className="p-6 text-center relative overflow-hidden">
              ...
              <p className="text-xs text-slate-500 uppercase tracking-widest mb-2">
                Pegada de Carbono - {getMesLabel(mesSel)}
              </p>
              ...
              {nivel && (
                <>
                  <span
                    className={clsx(
                      'inline-block text-sm font-bold px-4 py-1 rounded-full border mb-3',
                      nivel.bg,
                      nivel.color
                    )}
                  >
                  ...
                  <div className="w-full h-2 bg-white/10 rounded-full overflow-hidden mt-2">
                    <motion.div
                      initial={{ width: 0 }}
                      animate={{ width: `${Math.min((carbono.totalKg / 167) * 100, 100)}%` }}
                  ...
                  <p className="text-xs text-slate-500 mt-1.5">
                    Meta sustentavel: menos de 167 kg CO2/mes (2t/ano)
                  </p>
                </>
              )}
            </GlassCard>
              ...
              {[
                ...
                <motion.div key={c.label} whileHover={{ scale: 1.04 }}>
                  <GlassCard className="p-4 text-center">
                    <div
                      className={clsx(
                        'w-9 h-9 rounded-xl bg-gradient-to-br flex items-center justify-center mx-auto mb-2',
                        c.cor
                      )}
                    >
                  ...
                  <BarChart data={historico} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
                    ...
                    <Bar dataKey="kg" radius={[6, 6, 0, 0]}>
                      {historico.map((entry, i) => (
                        <Cell
                          key={i}
                          fill={entry.label === getMesLabel(mesSel, true) ? '#22c55e' : '#334155'}
                        />
                      ))}
                    </Bar>
                  </BarChart>
            ...
            <GlassCard className="p-5 space-y-3">
              ...
              <div className="grid sm:grid-cols-3 gap-3">
                {[
                  {
                    emoji: '🌳',
                    label: `${carbono.arvoresTotais} arvore${carbono.arvoresTotais !== 1 ? 's' : ''}`,
                    sub: 'para absorver em 1 ano',
                  },
                  {
                    emoji: '🚗',
                    label: `${carbono.kmCarro.toLocaleString('pt-BR')} km`,
                    sub: 'rodados de carro',
                  },
                  {
                    emoji: '✈️',
                    label: `${carbono.horasAviao}h de voo`,
                    sub: 'em altitude de cruzeiro',
                  },
                ].map((eq, i) => (
                  <motion.div
                    key={i}
            ...
            <GlassCard className="p-5 space-y-3 border-emerald-500/20 bg-emerald-600/5">
              ...
              {loadingIA ? (
                ...
                <motion.p
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                  className="text-sm text-emerald-200 leading-relaxed"
                >
                  {dicaIA}
                </motion.p>
              ) : null}
  ...
  async function handleArtigoLido(id: string) {
    ...
    setProgress((p) =>
      p
        ? {
            ...p,
            artigosLidos: [...p.artigosLidos, id],
            xpEducacao: p.xpEducacao + 50,
          }
        : p
    );
  }

  async function handleQuizCompleto(
    catId: string,
    acertos: number,
    total: number,
    xp: number
    ...
    setProgress((p) =>
      p
        ? {
            ...
            quizzesCompletos: [
              ...p.quizzesCompletos,
              { catId, acertos, total, data: new Date() },
            ],
  ...
  return (
    ...
    >
      ...
      {!loadingProgress && (
        ...
        >
          <div className="flex gap-4 flex-wrap">
            {[
              {
                label: 'Noticias lidas',
                value: `${artigosLidos}/${NOTICIAS_CATEGORIAS.length}`,
                icon: BookOpen,
                color: 'text-blue-400',
              },
              {
                label: 'Quizzes',
                value: `${quizCompletos}/${QUIZ_CATEGORIAS.length}`,
                icon: Brain,
                color: 'text-violet-400',
              },
              {
                label: 'Desafios',
                value: `${progress?.desafiosAceitos.length ?? 0}/7`,
                icon: Bolt,
              ...
              <div key={s.label} className="flex items-center gap-2">
                ...
                <span className={clsx('text-xs', darkMode ? 'text-slate-500' : 'text-slate-400')}>
                  {s.label}
                </span>
              </div>
            ))}
          ...
          <div className="flex-1 min-w-0">
            <div className={clsx('h-1.5 rounded-full', darkMode ? 'bg-white/10' : 'bg-slate-200')}>
              <motion.div
                ...
                animate={{
                  width: `${Math.round(
                    ((artigosLidos + quizCompletos) /
                      (NOTICIAS_CATEGORIAS.length + QUIZ_CATEGORIAS.length)) *
                      100
                  )}%`,
                }}
            ...
            <p className={clsx('text-[10px] mt-1', darkMode ? 'text-slate-500' : 'text-slate-400')}>
              Progresso geral
            </p>
          </div>
        </motion.div>
      ...
      <motion.div
        ...
        className={clsx(
          'flex gap-1 p-1.5 rounded-2xl overflow-x-auto border backdrop-blur-sm',
      ...
      <AnimatePresence mode="wait">
        ...
        >
          {tab === 'artigos' && (
            <ArtigosTab progress={progress} onArtigoLido={handleArtigoLido} />
          )}
          {tab === 'videos' && <VideosTab />}
          {tab === 'quiz' && (
            <QuizTab progress={progress} onQuizCompleto={handleQuizCompleto} />
          )}
